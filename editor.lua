local code = [[
	...
	PANEL.URL = "https://metastruct.github.io/lua_editor/"
	PANEL.COMPILE = "C"
	...
	ed:SetPos(5, 55)
	local mclos = vgui.Create("DButton", menu)
	mclos:SetSize(40,23)
	mclos:SetText("")
	mclos.Paint = function(self,w,h)
	...
	local function CreateMenuBtn(wide, text, icon, fn)
		...
		btn.DoClick = fn
		btn:SetSize(wide, 20)
		btn:SetPos(menuoffset, 30)
		menuoffset=menuoffset + wide + 5
	end

	CreateMenuBtn(90, "Self", 'icon16/user_gray.png', function()
		RunString(ed:GetCode())
	end)
	concommand.Add('editor', function(ply) menu:SetVisible(!menu:IsVisible()) end)
]]
